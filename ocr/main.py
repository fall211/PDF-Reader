...
def open_file_picker():
    ...
    page_count_label.configure(text="Loading PDF file...")
    threading.Thread(target=load_pdf, args=([file_path])).start()

def load_pdf(file_path):
    pdf2text.set_file_path(file_path)
    current_file_label.configure(text=file_path)
    page_count = pdf2text.get_page_count()
...
def process_text(start_page):
    ...
    controls_frame.pack()
    file_path = pdf2text.get_file_path()
    page = start_page - 1

    while page <= pdf2text.get_page_count() and is_processing:
        queue = audio_queue.get_queue()
        if len(queue) < 5:
            text = pdf2text.get_text_from_page(page)
            try:
